...
def git_clone_task(task: executor.Task) -> RepoInfo:
  ...
  fetch.fetch_and_build_projects([task.project], task.homedir, temp_dir_path, task.build)
...
def git_repos(repos: cl.Repos, projs_to_fetch: ty.List[str], build: bool) -> ty.Tuple[bool, ty.List[RepoInfo]]:
  ...
  if len(tasks) < 10:
    ...
    results = executor.execute_batch_parallel(
      tasks, git_clone_task, desc='git clone', transf_fn=lambda x: x)
  if len(results) == 0:
...
def git_repos_in_corpus(repos: cl.Repos, corpus_json_file: str, rs, build: bool) -> ty.Optional[ty.List[RepoInfo]]:
  ...
  if not os.path.exists(os.path.join(repos.homedir, "srcdir")):
    ...
    if len(results) == 0:
      return None
    logger.info("Recovered %d repos from '%s'", len(results), repos.homedir)
...
def fetch_repos_cmd(repos, rs, build):
  ...
  if repos_info_out is None:
    click.echo(f"Unable to download GitHub repositories in {corpus_json_file}.")
    return
...
def run_descert_cmd(repos):
  ...
  if not dljc_out or dljc_out is None:
    click.echo("Failed to run DesCert. Please check log for additional info.")
    return

...
def fetch_project(project_name, srcdir, tempdir):
  ...
  if project_dir:
    common.move(project_dir, os.path.join(os.path.join(srcdir, project["name"])))
...
def fetch_and_build_projects(projects, srcdir, tempdir, build=False):
  ...
  for project_name in projects:
    fetched_projects |= {fetch_single_project(project_name, srcdir, tempdir)}
    fetched_projects |= {build_single_project(project_name, srcdir, build=build)}
